...
#define NUM_CORES 4
uint128_t data0[NUM_CORES*NUM_CACHELINES] __attribute__((section(".cache_share_region")));
...
int writeback_evict(int cid, int nc)
{
  ...
  for (int i = 0; i < NUM_CACHELINES; i++) {
    data0[cid*NUM_CACHELINES+i] = cid*NUM_CACHELINES+i+1;
    if (data0[cid*NUM_CACHELINES+i] != cid*NUM_CACHELINES+i+1)
      exit(cid*NUM_CACHELINES+i+1);
  }

  // modify the cacheline
  for (int i = 0; i < NUM_CACHELINES; i++) {
    data0[cid*NUM_CACHELINES+i] *= 10;
    if (data0[cid*NUM_CACHELINES+i] != 10*(cid*NUM_CACHELINES+i+1))
